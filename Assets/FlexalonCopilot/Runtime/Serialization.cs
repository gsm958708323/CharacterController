...
namespace FlexalonCopilot
{
    internal static class Serialization
    {
        ...
        internal static bool TryDeserialize(Type type, string value, out object result)
        {
            ...
            #if UNITY_TMPRO
                else if (type == typeof(TMPro.TMP_Dropdown.OptionData))
                {
                    result = new TMPro.TMP_Dropdown.OptionData { text = value };
                    return true;
                }
            #endif
            else if (type.IsEnum)
            ...
            else if (type.IsSubclassOf(typeof(UnityEngine.Object)))
            {
                    ...
                    {
                        result = UnityEditor.EditorUtility.InstanceIDToObject(id);
                        return true;
                    }
                    else if (value.Length == 32)
                    {
                        var assetPath = UnityEditor.AssetDatabase.GUIDToAssetPath(value);
        ...
        internal static object DeserializeOrDefault(Type t, string value)
        {
        ...
        internal static T DeserializeOrDefault<T>(string value)
        {
            return (T)DeserializeOrDefault(typeof(T), value);
        }
