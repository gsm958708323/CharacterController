    ...
    internal class FastJson
    {
        ...
        private static bool TrySerializeValue(Type type, object data, bool quote)
        {
            switch (type)
            {
                ...
                #if UNITY_TMPRO
                    case Type t when t == typeof(TMPro.TMP_Dropdown.OptionData): SerializeOptionData(data, quote); break;
                #endif
        ...
        private static void SerializeString(object data, bool quote)
        {
            ...
            for (int i = 0; i < len; i++)
            {
                ...
                {
                    // TODO: We need to write the decoder too for this to work.
                    // needEncode = true;
                    break;
                }
            }
...
#if UNITY_TMPRO
        private static void SerializeOptionData(object data, bool quote)
        {
            var optionData = (TMPro.TMP_Dropdown.OptionData)data;
            SerializeString(optionData.text, quote);
        }
#endif
        ...
        {
            var unityObject = (UnityEngine.Object)data;
            string id = unityObject.GetInstanceID().ToString();
            #if UNITY_EDITOR
                var assetPath = UnityEditor.AssetDatabase.GetAssetPath(unityObject);
                id = string.IsNullOrEmpty(assetPath) ? id : UnityEditor.AssetDatabase.AssetPathToGUID(assetPath);
            #endif
            if (quote)
            {
                Write('"');
